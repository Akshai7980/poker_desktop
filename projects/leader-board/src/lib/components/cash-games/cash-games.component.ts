...
export class CashGamesComponent implements OnInit, OnDestroy {
  ...
  cashList: LeaderBoardListResponse[] = [];
    ...
    const getLeaderBoardList: Subscription = getLeaderBoardList$.subscribe({
      next: (res: BaseResponse<LeaderBoardListResponse[]>) => {
        if (res.code === Leaderboard.SUCCESS) {
          ...
          this.cashList = this.leaderBoardList.filter(
            (item: LeaderBoardListResponse) =>
              item.contestType.toLowerCase() === ConstantValues.CGP ||
              item.contestType.toLowerCase() === ConstantValues.CGC ||
              item.contestType.toLowerCase() === ConstantValues.POY
          );
          if (this.cashList?.length !== 0) {
            const data: LBDataTransformModel = {
              leagueId: this.cashList[0].leagueId,
              contestType: this.cashList[0].contestType,
              displayName: this.cashList[0].displayName,
              description: this.cashList[0].description,
              showRhs: true
            };
          ...
          this.joinedList = this.cashList.filter((item: LeaderBoardListResponse) => item.isJoined);
